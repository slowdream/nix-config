...
}:
{
  ...
  boot.kernelParams = [
    # === NVMe: таймауты / зависания в Linux ===
    # https://community.frame.work/t/nvme-timeout-woes/54999

    "nvme_core.default_ps_max_latency_us=0"
    # APST NVMe выкл.: диск не уходит в глубокий sleep под нагрузкой,
    # иначе пробуждение >30 ms → «таймаут» команд.

    "nvme_core.io_timeout=4294967295"
    # Максимальный таймаут команд NVMe (~49 дней) вместо 30 s по умолчанию —
    # ядро не сбрасывает контроллер при кратковременных задержках.

    "pcie_aspm=off"
    # ASPM для PCIe выкл. — частая причина таймаутов NVMe на Linux (L1/L1.2).
  ];
